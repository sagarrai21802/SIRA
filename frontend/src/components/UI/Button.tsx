...
import { LucideIcon } from 'lucide-react'; // ✅ Correct type import

interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'outline' | 'ghost';
  size?: 'sm' | 'md' | 'lg';
  icon?: LucideIcon; // ✅ Now it's a type, not a single icon
  loading?: boolean;
  asChild?: boolean;
...
export function Button({
  ...
  if (asChild) {
    ...
    return React.cloneElement(child, {
      className: child.props.className ? `${child.props.className} ${combinedClassName}` : combinedClassName,
