...
export default function PromptGeneratorUI() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await generatePrompt({ topic, type });
      setResult(res.prompt); // ✅ Only keep main prompt
    } catch (err) {
      console.error(err);
      setResult("⚠️ Failed to generate prompt. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-xl max-w-2xl mx-auto">
      {/* Tabs */}
      <div className="flex gap-3 mb-5">
        {["image", "video"].map((t) => (
          <button
      ...
      <button
        onClick={handleGenerate}
        disabled={loading}
        className="w-full px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-pink-500 text-white font-semibold shadow-lg hover:scale-105 transition"
      >
        {loading ? "Generating..." : "Generate Prompt"}
      </button>
      ...
      {/* Result */}
      {result && (
        <div className="mt-6 p-4 bg-gray-100 dark:bg-gray-800 rounded-xl relative shadow-md">
          <h3 className="font-bold mb-2 text-gray-800 dark:text-white">✨ Your Prompt</h3>
