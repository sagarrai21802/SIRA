...
export default function InstagramPostGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const template = await generateTemplate({
        ...
        tone: "creative", // Instagram is more fun + visual
        style: "instagram-post",
      });
      setResult(template);
    } catch (err) {
      console.error("Template generation failed:", err);
      alert("Error generating Instagram post. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const img = await generateTemplateImage(result.templateBody, "1080x1080"); // Insta square format
      setImage(img);
    } catch (err) {
      ...
      {result && (
        <Card className="mt-6 shadow-2xl rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl">
          <CardContent className="p-6 space-y-6">
            ...
            <p className="font-semibold text-pink-600 dark:text-pink-400">{result.callToAction}</p>

            {result.tips?.length > 0 && (
              <div className="bg-pink-50 dark:bg-pink-900/30 p-4 rounded-xl">
                <h3 className="text-sm font-semibold text-pink-700 dark:text-pink-300 mb-2">Quick Tips:</h3>
            ...
            {image && (
              <div className="mt-6">
                <h3 className="font-medium text-gray-800 dark:text-gray-200 mb-2">Generated Instagram Image:</h3>
                <img src={image} alt="Generated Instagram Post" className="rounded-xl shadow-lg w-full border border-gray-200 dark:border-gray-700" />
              </div>
            )}
          </CardContent>
        </Card>
