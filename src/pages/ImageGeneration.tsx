...
export const ImageGenerator: React.FC = () => {
  ...
  const loadGeneratedImages = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE || 'https://sira-msb1.onrender.com'}/api/image-generations?user_id=${user.id}`);
      if (response.ok) {
  ...
  const handleDeleteImage = async (imageId: string) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE || 'https://sira-msb1.onrender.com'}/api/image-generations/${imageId}`, {
        method: 'DELETE'
      });
